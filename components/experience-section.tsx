      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="relative">
          ...
          <div className="absolute left-6 top-0 hidden h-full w-px bg-secondary/50 md:left-1/2 md:block">
            <div
              className="w-full bg-linear-to-r from-primary via-accent to-primary transition-all duration-100 ease-out"
            ...
            {experiences.map((exp, i) => (
              <div
                ...
                className={`flex flex-col items-start gap-6 md:flex-row md:items-center ${i % 2 === 1 ? "md:flex-row-reverse" : ""
                  } transition-all duration-700 ease-in-out ${isVisible ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0"
                  }`}
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                {/* Card */}
                <div
                  className={`glass group w-full rounded-xl p-6 transition-all duration-500 ease-in-out hover:border-primary/30 hover:shadow-[0_0_30px_rgba(56,189,248,0.15)] md:w-[calc(50%-2rem)] ${isVisible
                ...
                {/* Timeline dot */}
                <div
                  className={`hidden h-4 w-4 shrink-0 rounded-full border-2 border-primary bg-background transition-all duration-500 ease-in-out md:block ${scrollProgress > (i + 0.5) / experiences.length
